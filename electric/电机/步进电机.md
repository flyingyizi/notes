...
约定：
- $vlim = max(q˙(t))$: the maximum value of the velocity during the motion. 走完路径h中的最大速度。

显然会存在两种case：

- Case 1: $vlim = vmax$.
- Case 2: $vlim < vmax$. 对速度v只有加速阶段与减速阶段，没有等速阶段。

## 在给定约束下，完成位移h是否可行？

约束包括:$v_{max},a_{max},j_{max},$, 要在该约束下完成指定位移($h)，讨论完成的可行性时，显然就是识别是否可以在单纯加速 (if v0 < v1) 或单纯减速(if v0 > v1) 情况下是否可以在不突破约束情况下完成指定位移。

从速度0到速度$v_{max}$,需要耗费时间是$t=\cfrac{v_{max}}{a_{max}}$
...
T^*_j *j_{max} 与 \cfrac{|v_1-v_0|}{T^*_j}
...
$$
 ...
 a'= jerk = (v-v_0)(t^{-1})' = \cfrac{v_0-v}{t^2} 
$$

[基于S曲线的步进电机加减速的控制](https://wenku.baidu.com/view/2c78c4a872fe910ef12d2af90242a8956becaa6a.html)
...
& h_{max}  ,t\in [0,t_I) \\
& 0        ,t\in [t_I,t_{II}) \\
& -h_{max} ,t\in [t_{II},t_{III}) \\
& 0        ,t\in [t_{III},t_{IV}) \\
& -h_{max} ,t\in [t_{IV},t_{V}) \\
& 0        ,t\in [t_{V},t_{VI}) \\
& h_{max}  ,t\in [t_{VI},t_{VII}) \\
...
v(t)
\begin{cases}
& v_0 + \cfrac{1}{2}h_{max}*t^2  ,t\in [0,t_I) \\
& v_1 + a_{max}*t                ,t\in [t_I,t_{II}) \\
& -h_{max} ,t\in [t_{II},t_{III}) \\
& 0        ,t\in [t_{III},t_{IV}) \\
& -h_{max} ,t\in [t_{IV},t_{V}) \\
& 0        ,t\in [t_{V},t_{VI}) \\
