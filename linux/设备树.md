...
- `clocks` 属性: 格式为"`clocks = <phandle1 specifier1 [phandle2 specifier2] [phandle3 specifier3] ... >`"	List of phandle and clock specifier pairs, one pair
		for each clock input to the device.  Note: if the
		clock provider specifies '0' for #clock-cells, then
		only the phandle portion of the pair will appear.

        该属性仅出现在clock consumer节点中。

- `#clock-cells` 属性： Number of cells in a clock specifier; Typically 0 for nodes
		   with a single clock output and 1 for nodes with multiple
		   clock outputs.

           该属性出现在clock provider节点中。
...
Optional properties:
- clock-output-names: 时钟输出信号名称列表，“clock consumer”不会直接使用它，仅仅会使用列表索引。
